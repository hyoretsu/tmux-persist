...
save leaky
# _PERSIST_TIMESTAMP has 1-second resolution - without this, a fast runner can
# land the simulated failed save in the same second as the real one above,
# reusing its exact filename and legitimately deleting ITS companion instead
# of a newly-leaked one (same pattern as test_skip_unchanged.sh).
sleep 1.2
